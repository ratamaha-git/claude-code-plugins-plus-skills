...
Update the task file to `completed` or `blocked` with concise verification evidence and commit refs. Append only durable verified project learnings to the relevant `.hyperflow/memory/*.md` file. Planning decisions are recorded only through the explicit `plan --remember` contract; accepted review outcomes use the explicit audit `--remember` contract and leave the review audit as the source of truth.
